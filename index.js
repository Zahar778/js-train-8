...
function combineAndReverseArrays(arr1, arr2) {
  if (Array.isArray(arr1, arr2) === true) {
...
function findElementIndexes(arr, element) {
  if (Array.isArray(arr) === true) {
    let indexferst = arr.indexOf(element);
    let indexalst = arr.lastIndexOf(element)
    let a = [indexferst , indexalst];
    return a;
  }else 
  []
...
function copyAndSwapElements(arr, target, start, end) {
  if (start < 0 || end > arr.length || target < 0 || target >= arr.length || start >= end) {
 ...
 */
/**
 * Функція `sortByKey` сортує вхідний масив об'єктів за допомогою заданого ключа.
 *  arr - Вхідний масив об'єктів.
 *  key - Ключ, за яким потрібно сортувати об'єкти.
 * Повертає: Відсортований масив об'єктів.
 */
...
function customEvery(arr, condition) {
  if ( Array.isArray(arr) === false ) {
    return false
  }else if (Array.isArray(condition) === true ) {
    return false
  }
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  if ( Array.isArray(arr) === false ) {
    return false;
  }else if (start < 0 || end > arr.length  || start >= end) {
    return false;
  }
...
function customShift(arr) {
  ...
  let shiftedElement = arr.shift(0);
...
function customAt(arr, index) {
  ...
  // Визначаємо тип елементу і виводимо відповідне повідомлення
